...
/**
 * Props for SftpConnectionDialog component
 */
interface SftpConnectionDialogProps {
  open: boolean;
...
/**
 * SFTP Connection Dialog Component
 *
 * Provides a UI for connecting to SFTP servers with password or private key authentication.
...
export function SftpConnectionDialog({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      console.log('New SFTP profile created with ID:', profileId);
      ...
      try {
        ...
        if (formState.authMethod === 'password' && formState.password) {
          await saveCredential(profileId, 'sftp', 'password', formState.password); // ← 'sftp' service
          console.log('Saved password to keyring');
        }

        if (formState.authMethod === 'privateKey' && formState.privateKeyPath) {
          await saveCredential(profileId, 'sftp', 'privatekey', formState.privateKeyPath); // ← 'sftp' service
          console.log('Saved private key to keyring');
        }

        if (formState.passphrase) {
          await saveCredential(profileId, 'sftp', 'passphrase', formState.passphrase); // ← 'sftp' service
      ...
      toast.success('Connecting...', {
        description: `Connecting to SFTP ${formState.username}@${formState.host}`,
      });
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Connect to a remote server via SFTP</DialogDescription>
        </DialogHeader>
